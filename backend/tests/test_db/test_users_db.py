...
def test_get_user_with_id(user_db: UsersDBMySql, existing_user: MockUser):
    user = user_db.get_user_with_id(existing_user.user_id)
...
def test_get_user_with_email(user_db: UsersDBMySql, existing_user: MockUser):
    user = user_db.get_user_with_email(existing_user.email)
...
def test_get_user_invalid_user_id(user_db: UsersDBMySql):
    with pytest.raises(UserNotFoundError):
        user_db.get_user_with_id("invalid")
...
def test_add_user(user_db: UsersDBMySql):
    ...
    new_user = user_db.get_user_with_id(new_user.user_id)
...
def test_edit_user(user_db: UsersDBMySql):
    ...
    edited_user = user_db.get_user_with_id(user_id)
...
def test_delete_user(user_db: UsersDBMySql):
    ...
    with pytest.raises(UserNotFoundError):
        user_db.get_user_with_id(user.user_id)
